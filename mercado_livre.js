...
async function main() {
  ...
  await Promise.all([
      ...
      //função para clicar
      await page.click('body > header > div > div.nav-area.nav-top-area.nav-center-area > form > button > div')
  ]);
  
  //função para pegar todos os produtos do site e jogar em um array
  const links = await page.$$eval('.ui-search-item__group.ui-search-item__group--title > a', el => el.map(link => link.href));
  ...
  //console.log(links);
   ...
   for(const link of links){
    ...
    try{
      title = await page.$eval('.ui-pdp-header__title-container', element => element .innerText);
     ...
     try{
      numAval = await page.$eval(".ui-pdp-review__amount", element => element .innerText);
